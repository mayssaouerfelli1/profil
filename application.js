...
import { faUser, faTrash, faEdit, faFileAlt, faPlus, faEye, faFileVideo, faFileAudio, faFile, faBook } from '@fortawesome/free-solid-svg-icons';
...
const Application = () => {
  ...
  const [option1, setOption1] = useState('videos');
...
*/
  ...
  const transcribeFile = async (file, langueOriginal, id_fichier) => {
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/transcribeFichier', {
        "filePath": "C://Users//moi//extraction-project//backend_pfe//public//uploads//" + file,
  ...
  const transcribeAudioVideoFile = async (file, selectedOriginalLanguage) => {
    try {
      const formData = new FormData();
      ...
      if (result.success) {
        console.log('Contenu du fichier original :', result.contentOriginal);
        alert('Fichier transcrit avec succès !');
      } else {
        alert('Erreur lors de la transcription du fichier : ' + result.message);
      }
    } catch (error) {
      alert('Erreur lors de la transcription du fichier : ' + error.message);
    }
  };
